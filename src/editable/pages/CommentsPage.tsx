    ...
    <EditableSiteShell>
      <main className="bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="mx-auto max-w-[1180px] px-4 py-10 sm:px-6 lg:px-8">
          <div className="rounded-md border border-black/10 bg-[var(--slot4-surface-bg)] p-6 shadow-[0_10px_28px_rgba(0,0,0,0.08)] sm:p-8">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              ...
              <button type="button" className="rounded-sm border border-black/10 px-4 py-2 text-sm font-black" onClick={refreshComments}>Refresh comments</button>
            </div>
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="relative w-full sm:max-w-md">
                ...
                <input
                  ...
                  className="h-11 w-full rounded-sm border border-black/10 bg-[var(--slot4-surface-bg)] pl-9 pr-3 text-sm outline-none"
                />
              </div>
          ...
          {visibleComments.length ? (
            <div className="mt-8 grid gap-4">
              {visibleComments.map((item) => (
                <article key={`${item.articleSlug}-${item.id}`} className="rounded-md border border-black/10 bg-[var(--slot4-surface-bg)] p-5 shadow-sm">
            ...
            <div className="mt-8 rounded-md border border-dashed border-black/20 bg-[var(--slot4-surface-bg)] p-8 text-center">
          ...
          {filtered.length > COMMENTS_PER_PAGE ? (
            <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-md border border-black/10 bg-[var(--slot4-surface-bg)] p-4 text-sm text-[var(--slot4-muted-text)]">
              <span>Page {currentPage} of {totalPages}</span>
              <div className="flex gap-2">
                <button type="button" className="rounded-sm border border-black/10 px-4 py-2 font-black disabled:opacity-40" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</button>
                <button type="button" className="rounded-sm border border-black/10 px-4 py-2 font-black disabled:opacity-40" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Next</button>
              </div>
            </div>
          ) : null}
        </section>
