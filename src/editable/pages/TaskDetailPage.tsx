...
function BackLink({ task }: { task: TaskKey }) {
  ...
  return (
    <Link href={taskConfig?.route || '/'} className="inline-flex items-center gap-2 rounded-full border border-black/10 bg-white/70 px-4 py-2 text-sm font-black">
...
function ArticleDetail({ post, related, comments }: { post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="mx-auto grid max-w-[1180px] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_350px] lg:px-8 lg:py-16">
      <article className="min-w-0 rounded-[2.7rem] border border-black/10 bg-[var(--slot4-surface-bg)] p-5 shadow-[0_10px_28px_rgba(0,0,0,0.08)] sm:p-8 lg:p-12">
  ...
  return (
    <section className="mx-auto max-w-[1180px] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_420px]">
        <article className="rounded-[2.8rem] border border-black/10 bg-[var(--slot4-surface-bg)] p-6 text-[var(--slot4-page-text)] shadow-[0_10px_28px_rgba(0,0,0,0.08)] sm:p-9">
          <div className="grid gap-6 sm:grid-cols-[150px_1fr]">
            <div className="flex h-36 w-36 items-center justify-center overflow-hidden rounded-[2rem] bg-[var(--slot4-page-bg)] ring-1 ring-black/10">
...
function ClassifiedDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[1180px] gap-7 px-4 py-10 sm:px-6 lg:grid-cols-[0.82fr_1.18fr] lg:px-8 lg:py-16">
      <aside className="rounded-[2.5rem] border border-black/10 bg-[var(--slot4-dark-bg)] p-7 text-[var(--slot4-dark-text)] shadow-xl lg:sticky lg:top-24 lg:self-start">
      ...
      <article className="rounded-[2.7rem] border border-black/10 bg-[var(--slot4-surface-bg)] p-6 text-[var(--slot4-page-text)] shadow-[0_10px_28px_rgba(0,0,0,0.08)] sm:p-9">
  ...
  return (
    <section className="mx-auto max-w-[1180px] px-4 py-10 sm:px-6 lg:px-8 lg:py-16">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[0.72fr_1.28fr]">
        <aside className="rounded-[2.5rem] border border-black/10 bg-[var(--slot4-surface-bg)] p-7 text-[var(--slot4-page-text)] lg:sticky lg:top-24 lg:self-start">
        ...
        <div className="columns-1 gap-5 space-y-5 md:columns-2">
          {(images.length ? images : ['/placeholder.svg?height=900&width=1200']).map((image, index) => (
            <figure key={`${image}-${index}`} className="break-inside-avoid overflow-hidden rounded-[2rem] border border-black/10 bg-[var(--slot4-surface-bg)] shadow-sm">
...
function BookmarkDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[1180px] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[2.7rem] border border-black/10 bg-[var(--slot4-surface-bg)] p-7 text-[var(--slot4-page-text)] shadow-[0_10px_28px_rgba(0,0,0,0.08)] sm:p-10">
        <BackLink task="sbm" />
  ...
  return (
    <section className="mx-auto grid max-w-[1180px] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[minmax(0,1fr)_360px] lg:px-8 lg:py-16">
      <article className="rounded-[2.7rem] border border-black/10 bg-[var(--slot4-surface-bg)] p-6 text-[var(--slot4-page-text)] shadow-[0_10px_28px_rgba(0,0,0,0.08)] sm:p-9">
        ...
        <BodyContent post={post} />
        {fileUrl ? (
          <div className="mt-8 overflow-hidden rounded-[2rem] border border-black/10 bg-[var(--slot4-page-bg)]">
            <div className="flex items-center justify-between gap-3 border-b border-black/10 bg-[var(--slot4-surface-bg)] p-4">
...
function ProfileDetail({ post, related }: { post: SitePost; related: SitePost[] }) {
  ...
  return (
    <section className="mx-auto grid max-w-[1180px] gap-8 px-4 py-10 sm:px-6 lg:grid-cols-[420px_minmax(0,1fr)] lg:px-8 lg:py-16">
      <aside className="rounded-[2.7rem] border border-black/10 bg-[var(--slot4-surface-bg)] p-8 text-center text-[var(--slot4-page-text)] shadow-[0_10px_28px_rgba(0,0,0,0.08)] lg:sticky lg:top-24 lg:self-start">
        <BackLink task="profile" />
        <div className="mx-auto mt-10 flex h-40 w-40 items-center justify-center overflow-hidden rounded-full bg-[var(--slot4-page-bg)] ring-1 ring-black/10">
      ...
      <article className="rounded-[2.7rem] border border-black/10 bg-[var(--slot4-surface-bg)] p-7 text-[var(--slot4-page-text)] shadow-sm sm:p-10">
        <BodyContent post={post} />
...
function BodyContent({ post, compact = false }: { post: SitePost; compact?: boolean }) {
  return <div className={`article-content mt-8 max-w-none ${compact ? 'text-base leading-8' : 'text-lg leading-9'} text-[var(--slot4-soft-muted-text)]`} dangerouslySetInnerHTML={{ __html: formatPlainText(getBody(post)) }} />
}
  ...
  return (
    <div className="mt-8 grid gap-3 sm:grid-cols-2">
      {visible.map(([label, value, Icon]) => (
        <div key={label} className="rounded-[1.5rem] border border-black/10 bg-[var(--slot4-panel-bg)] p-4">
...
function MapBox({ src, label }: { src: string; label: string }) {
  return (
    <div className="overflow-hidden rounded-[2rem] border border-black/10 bg-[var(--slot4-surface-bg)] shadow-sm">
...
function ContactAction({ website, phone, email }: { website?: string; phone?: string; email?: string }) {
  ...
  return (
    <div className="mt-5 rounded-[2rem] border border-black/10 bg-[var(--slot4-surface-bg)] p-5 text-[var(--slot4-page-text)] shadow-sm">
...
function RelatedPanel({ task, post, related, compact = false }: { task: TaskKey; post: SitePost; related: SitePost[]; compact?: boolean }) {
  ...
  return (
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="rounded-[2rem] border border-black/10 bg-[var(--slot4-panel-bg)] p-5">
          <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--slot4-muted-text)]">About this post</p>
          <div className="mt-4 grid gap-3 text-sm font-bold text-[var(--slot4-soft-muted-text)]">
...
function RelatedCard({ task, post }: { task: TaskKey; post: SitePost }) {
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 rounded-2xl border border-black/10 bg-[var(--slot4-surface-bg)] p-3 transition hover:-translate-y-0.5 hover:shadow-lg">
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-10 rounded-[2rem] border border-black/10 bg-[var(--slot4-panel-bg)] p-5">
      <div className="flex items-center gap-2 text-lg font-black"><MessageCircle className="h-5 w-5" /> Comments</div>
      <div className="mt-5 grid gap-3">
        {comments.slice(0, 5).map((comment) => (
          <div key={comment.id} className="rounded-2xl border border-black/10 bg-[var(--slot4-surface-bg)] p-4">
